...
class UR10e:
    ...
    # Written against self.A/D/ALPHA/MASS/COM (not module globals) so both
    # UR10e and UR5e (or any other ROBOT subclass) get this for free.
    ...
    def _frames_batch(self, q: np.ndarray) -> list[np.ndarray]:
        ...
        for i in range(6):
            frames.append(frames[-1] @ self._dh_batch(q[:, i], self.A[i], self.D[i], self.ALPHA[i]))
        return frames
    ...
    def gravity_batch(self, q: np.ndarray) -> np.ndarray:
        ...
        for i in range(6):
            com_h = np.einsum("nij,j->ni", frames[i + 1], np.append(self.COM[i], 1.0))
            Jv = self._point_jacobian_batch(frames, com_h[:, :3], up_to=i + 1)[:, :3]
            tau -= self.MASS[i] * np.einsum("nkj,k->nj", Jv, g_vec)
        if self.payload > 0.0:
