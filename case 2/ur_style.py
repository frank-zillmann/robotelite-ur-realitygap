...
def sequential_cmap():
    """Brand-consistent continuous colormap (light -> dark) for heatmaps and
    value-gradient plots, in place of a non-brand default like viridis/magma
    -- built from the palette itself so a gradient plot still reads as the
    same navy/blue family as every bar/line chart next to it.
    """
    return LinearSegmentedColormap.from_list("ur_blues", [GRID, LIGHT_BLUE, BLUE, NAVY])